...
# Objective function consists of the MSE normalized by the variance of the membrane potential 
...
def mse_var(pred, truth):
	pred = T.reshape(pred, (pred.shape[0], pred.shape[1]/10000, 10000))
	truth = T.reshape(truth, (truth.shape[0], truth.shape[1]/10000, 10000))
	mse_slice = (T.sqr(pred - truth)).mean(axis = -1)
	return T.sum(mse_slice/var, axis = -1)

def mse_var_avg(y_pred, y_true):
	y_predlp = filt_app(y_pred)
	y_lp = filt_app(y_true)
...
def model1D(X_train, y_train, X_test, num_layers):
	...
	graph.compile('rmsprop', {'loss':mse_var_avg})
	history = LossHistory()
